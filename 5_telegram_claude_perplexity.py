...
async def query_perplexity():
    ...
    try:
        ...
        messages = [
            {
                "role": "system",
                "content": dani_financial_description
            },
            {
                "role": "user",
                "content": dani_perplexity_prompt
            }
        ]
...
def create_prompt_for_fix_hebrew(text: str) -> str:
        return f"""
...
def fix_hebrew_text(text: str, character_description) -> str:
        try:
            client = Anthropic(api_key=ANTHROPIC_API_KEY)
            # Create the message
            message = client.messages.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": create_prompt_for_fix_hebrew(text)
                    }
                ]
            )
            ...
            raise Exception(f"Error correcting Hebrew text: {str(e)}")


async def main():
    
    # Check if today is the 18th of the month
    if datetime.now().day == 18:
        print("Running Perplexity query for monthly report...")
        perplexity_response = await query_perplexity()
        monthly_report = fix_hebrew_text(perplexity_response, dani_financial_description)
        await send_text_message(monthly_report)
